...
class DuelingQNet(nn.Module):
    ...
    def __init__(self, game, q_args, device):
        super().__init__()

        self._q_args = q_args
        self._n_actions = game.num_distinct_actions()

        self._relu = nn.ReLU(inplace=False)

        MPM = q_args.mpm_args.get_mpm_cls()
        self._mpm = MPM(game=game, device=device, mpm_args=q_args.mpm_args)
    ...
    def forward(self, info_state, legal_action_masks):
        shared_out = self._mpm(info_state=info_state)
    ...
    def get_adv(self, info_state, legal_action_masks):
        shared_out = self._mpm(info_state=info_state)
