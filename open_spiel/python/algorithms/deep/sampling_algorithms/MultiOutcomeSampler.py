...
class MultiOutcomeSampler(SamplerBase):
    """
    ...
    def _traverser_act(self, current_state, traverser, trav_depth, iteration_strats, sample_reach, cfr_iter):
        ...
        legal_action_mask = utils.get_legal_action_mask_torch(n_actions=self._game.num_distinct_actions(),
                                                              ...
                                                              dtype=torch.float32)
        info_state = np.array(current_state.information_state_tensor(traverser))
        ...
        actions = np.random.choice(self._actions_arranged, p=sample_strat, size=n_actions_to_smpl)

        # """""""""""""""""""""""""
        # Create next states
        # """""""""""""""""""""""""
        cumm_rew = 0.0
        aprx_imm_reg = torch.zeros(size=(self._game.num_distinct_actions(),),
                                   dtype=torch.float32,
                                   device=self._adv_buffers[traverser].device)
        ...
        for _c, a in enumerate(actions):
            ...
            _u_a = next_state.returns()[traverser]
            ...
            if not _done:
                _u_a += self._recursive_traversal(current_state=next_state,
                                                  traverser=traverser,
    ...
    def _any_non_traverser_act(self, current_state, traverser, trav_depth, iteration_strats,
        ...
        self.total_node_count_traversed += 1
        return super()._any_non_traverser_act(current_state=current_state,
                                              traverser=traverser,
